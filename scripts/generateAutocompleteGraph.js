...
function addPrefixFirst(prefix, number, profFirst, profLast) {
    ...
    var classNodeFirstChar = addSearchQueryCharacter(prefixSpaceNode, number.toString()[0]);
    if (!graph.hasEdge(prefixNode, classNodeFirstChar)) {
        graph.addEdge(prefixNode, classNodeFirstChar);
    }
    var classNode = addSearchQueryCharacter(classNodeFirstChar, number.toString().slice(1), {
        prefix: prefix,
        number: number
    });
    var classSpaceNode = addSearchQueryCharacter(classNode, ' ');
    var professorFirstNameNode = addSearchQueryCharacter(classSpaceNode, profFirst + ' ');
    var professorLastNameFirstCharNode = addSearchQueryCharacter(classSpaceNode, profLast[0]);
...
for (var i = 0; i < myPrefixes.length; i++) {
    ...
    for (var j = 0; j < myPrefixes[i].classes.length; j++) {
        ...
        for (var k = 0; k < myPrefixes[i].classes[j].professors.length; k++) {
            ...
            addPrefixFirst(myPrefixes[i].value, myPrefixes[i].classes[j].number, myPrefixes[i].classes[j].professors[k].firstName, myPrefixes[i].classes[j].professors[k].lastName);
            addProfFirst(myPrefixes[i].value, myPrefixes[i].classes[j].number, myPrefixes[i].classes[j].professors[k].firstName, myPrefixes[i].classes[j].professors[k].lastName);
        }
    }
}
fs.writeFileSync('data/autocomplete_graph.json', JSON.stringify(graph["export"]()));
console.log("Generated a ".concat(process.env.VERCEL_ENV === 'production' ? 'crawlable' : 'non-crawlable', " public/robots.txt"));
