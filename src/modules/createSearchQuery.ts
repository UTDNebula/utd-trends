    ...
    .flatMap((searchTerm) => {
      if (!(searchQueryLabel(searchTerm) in comboTable)) {
        if (
          searchTerm.prefix &&
          searchTerm.number &&
          /^(([0-9]X{3})|([0-9]{2}X{2}))$/.test(searchTerm.number)
        ) // this is an aggregate query, eg CS23XX
        {
          ...
          if (filterTerms.length > 0) {
            ...
            const allCombos: SearchQuery[] = courseQueries.flatMap(
              (courseQuery) => {
                return comboTable[searchQueryLabel(courseQuery)].map(
                  (combo) => ({
                    ...courseQuery,
                    ...combo,
                  }),
