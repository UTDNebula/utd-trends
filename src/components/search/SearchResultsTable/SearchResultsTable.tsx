            ...
            >
                ...
                <TableCell align="center" className="border-b-0">
                    {((typeof grades === 'undefined' || grades.state === 'error') && <></>) ||
                        ...
                        (grades.state === 'done' && (
                            <Tooltip title={'GPA: ' + grades.data.filtered.gpa.toFixed(2)} placement="top">
                                <Typography
                                    className="text-base text-black text-center rounded-full px-5 py-2 w-16 block mx-auto"
                                    sx={{
                                        backgroundColor: gpaToColor(rainbowColors, grades.data.filtered.gpa),
                                    }}
                                >
                ...
                <TableCell align="center" className="border-b-0">
                    {((typeof rmp === 'undefined' || rmp.state === 'error') && <></>) ||
                        ...
                        (rmp.state === 'done' && rmp.data.numRatings != 0 && (
                            <Tooltip title={'Professor rating: ' + rmp.data.avgRating} placement="top">
                                <div>
                                    <Rating defaultValue={rmp.data.avgRating} precision={0.1} sx={{ fontSize: 25 }} readOnly />
                                </div>
                            </Tooltip>
                        )) ||
                        null}
                ...
                <TableCell className="p-0" colSpan={6}>
                    <Collapse in={open} timeout="auto" unmountOnExit>
                        <div className="p-2 md:p-4 flex flex-col gap-2">
                            <SingleGradesInfo course={course} grades={grades} gradesToUse="filtered" />
...
const SearchResultsTable = ({
    ...
    const sortedResults = includedResults.sort((a, b) => {
        ...
        if (orderBy === 'rating') {
            ...
            const aRating = aRmp?.data?.avgRating ?? 0;
            const bRating = bRmp?.data?.avgRating ?? 0;
            if (order === 'asc') {
