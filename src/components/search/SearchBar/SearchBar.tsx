  ...
  function filterOptions(
    ...
    if (newInputValue.length >= 3) {
      ...
      if (prefixMatch && numberMatch && numberMatch[0].length < 3) {
        ...
        if (
          prefix in aggregate_table &&
          firstDigit in aggregate_table[prefix]
        ) {
          ...
          if (coursesCount > 1) {
            const aggregateOption: SearchQueryWithTitle = {
              subtitle: `All ${prefix} courses starting with ${numberMatch[0]}`,
              prefix: prefix,
              number: number,
            };
            return [...matchedRecents, aggregateOption, ...filtered]; // add aggregate to beginning of filtered list
          }
        }
      }
    }
