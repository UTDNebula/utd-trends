  ...
  <div style={{ width: myWidth, height: '300px' }}>
    {currentForm !== 'Radial' || currentData[0].data.length === 1 ? ( //if Radial AND too much data, give error block instead of GraphChoice component
      ...
      >
        <div>Uh oh..</div>
        <div>
          Radial graph <code>data</code> arrays need to only have one entry!
        </div>
      </div>
    )}
  </div>
  <div
    style={{ display: 'flex', flexDirection: 'horizontal', margin: '0.5rem' }}
  >
    <button
      style={{
      ...
      onClick={(e) => changeToNonRadial('Bar')}
    >
      Bar
    </button>
    <button
      ...
      onClick={(e) => changeToNonRadial('Vertical')}
    >
      Vertical
    </button>
    <button
      ...
      onClick={(e) => changeToNonRadial('Horizontal')}
    >
      Horizontal
    </button>
    <button
      ...
      onClick={(e) => changeToNonRadial('Line')}
    >
      Line
    </button>
    <button
      ...
      onClick={(e) => changeToNonRadial('Radar')}
    >
      Radar
    </button>
    <button
    ...
    <button
      ...
      onClick={(e) => changeToNonRadial('BoxWhisker')}
    >
      BoxWhisker
    </button>
  </div>
  ...
  >
    ...
    <button
      style={{
        ...
        margin: '0.65rem',
        padding: '0.5rem',
        backgroundColor: !radialFlag ? '#fff' : '#f5c4be',
      }}
      onClick={(e) => shrinkData()}
    >
      Change data to a set with one entry each
    </button>
  </div>
  <div style={{ margin: '1.5rem', marginTop: '2.5rem' }}>
    Current props being passed to the <code>GraphChoice</code> component:
    <div
      style={{
        border: '1px solid black',
        borderRadius: '6px',
    ...
    >
      <code>
        ...
        >
          <strong>form</strong> = "{currentForm}"
        </div>
        <div
          style={{ magin: '0.5rem', padding: '0.25rem', marginLeft: '2rem' }}
        >
          <strong>xaxisLabels</strong> = [
          {startAxisLabel.map((label) => label + ', ')}]
        </div>
        <div
          style={{
            magin: '0.5rem',
