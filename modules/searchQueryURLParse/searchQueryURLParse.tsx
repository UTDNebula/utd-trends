...
export function searchQueryURLParseMultiple(
  encodedSearchTerms: string | string[],
): SearchQuery[] {
  if (typeof encodedSearchTerms === 'string') {
    return encodedSearchTerms
      .split(',')
      .map((term) => searchQueryURLParse(term));
  } else {
    return encodedSearchTerms.map((term) => searchQueryURLParse(term));
  }
}

export default function searchQueryURLParse(
